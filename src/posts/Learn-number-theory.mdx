# Learn (almost) every Number Theory concept you'll need to know for Competitive Programming from a single problem!
...
- To calculate the multiplicative inverse when $m$ is prime, we use **Fermat's Little Theorem**:
  ...
  - If $p$ is a prime and $b \not\equiv 0 \pmod{p}$ (i.e., $b$ is not divisible by $p$), then:
    $$
b^{p - 1} \equiv 1 \pmod{p} \quad \text{(i.e., raising $b$ to the power $p-1$ leaves a remainder of 1 when divided by $p$)}
    $$
...
1.  Easy Version – https://www.codechef.com/problems/DIVISORS2
2.  Hard Version – https://www.codechef.com/problems/LOLBSGNJ6PK8
